...
class RegPePG():
    def __init__(self, env: Gridworld, max_iterations, lamda, reg, gradient, eta, sampling, 
                 n_sample, policy_gradient, nu, unregularized_obj, lagrangian, N, delta, B,
                 feature_dim=32, nn_lr=0.001, warmup=1, num_trajectories=30, value_lr=0.1, 
                 entropy_reg_lambda=0.1, seed=1):
        ...
        if not wandb.run:
            wandb.init(project="regpepg", name=f"regpepg_eta{eta}_nu{nu}_warmup_{warmup}_entreg{entropy_reg_lambda}")
    ...
    def _init_policy_parameterization(self):
        ...
        self.phi = np.random.normal(0, 0.2, (self.env.dim, num_actions, self.feature_dim))
    ...
    def _fit_value_function(self, trajectories):
        ...
        if self.iteration % 10 == 0:
            print(f"  Value function training: loss = {avg_loss:.6f}, samples = {len(states)}")
    ...
    def _compute_policy(self):
        ...
        for s in range(self.env.dim):
            logits = np.array([np.dot(self.theta, self.phi[s, a]) for a in range(num_actions)])
    ...
    def _collect_trajectories(self):
        ...
        for _ in range(self.num_trajectories):
            ...
            for t in range(200):
                ...
                action = np.random.choice(len(self.env.agents[1].actions), 
                                        p=self.pi_theta[state])
    ...
    def retrain1(self):
        ...
        if self.iteration < self.warmup:
            ...
            gradient = self._compute_entropy_regularized_gradient(trajectories, performative=False)
        
        elif self.iteration == self.warmup:
            print(f"Initial training of performative models at iteration {self.iteration}...")
            self._train_performative_models()
            gradient = self._compute_entropy_regularized_gradient(trajectories, performative=True)
        ...
        if self.iteration % 10 == 0:
            ...
            print(f"Iter {self.iteration}: d_diff = {self.d_diff[-1]:.6f}, grad_norm = {grad_norm:.6f}, theta_norm = {theta_norm:.6f}")
        
        self.iteration += 1

    def _compute_entropy_regularized_advantage(self, state, action, return_val, value_estimates):
        """
        ...
        log_pi = np.log(self.pi_theta[state, action] + 1e-8)  # Add small epsilon for numerical stability
    ...
    def _compute_entropy_regularized_gradient(self, trajectories, performative=False):
        ...
        for trajectory, returns, reg_advantages in trajectory_data:
            for t, ((state, action, reward), reg_advantage) in enumerate(zip(trajectory, reg_advantages)):
                ...
                for a_prime in range(len(self.env.agents[1].actions)):
                    grad_log_pi -= self.pi_theta[state, a_prime] * self.phi[state, a_prime]
                ...
                if performative:
                    ...
                    if t < len(trajectory) - 1:
                        ...
                        trans_grad = self._compute_transition_gradient(state, action, next_state)
        ...
        if self.iteration % 10 == 0 and all_returns:
            ...
            print(f"  Entropy-regularized gradient: avg_return={avg_return:.4f}, avg_reg_advantage={avg_reg_advantage:.4f}, reg_advantage_std={reg_advantage_std:.4f}")
    ...
    def _update_metrics(self):
        ...
        d_diff = np.linalg.norm(d_new - self.d_last) / (np.linalg.norm(self.d_last) + 1e-8)
    ...
    def _retrain_standard(self):
        ...
        for s in range(self.env.dim):
            if self.env.is_terminal(s): 
                continue
            constraints.append(cp.sum(d[s]) == self.env.rho[s] + 
        ...
        if d.value is not None:
            self.d_diff.append(np.linalg.norm(d.value - self.d_last) / (np.linalg.norm(self.d_last) + 1e-8))
            self.d_last = d.value
