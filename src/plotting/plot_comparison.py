...
from src.plotting.plot_utils import (
    get_algorithm_colors, get_algorithm_styles,
    plot_with_confidence_interval, style_axes, save_figure
)
...
def plot_algorithm_comparison(
    ...
    for param_val in all_param_values:
        _plot_algorithm_data(ax1, ax2, pg_by_param, param_val, 'pg',
                            'RPO FS', colors['pg'], styles['pg'])
        _plot_algorithm_data(ax1, ax2, pepg_by_param, param_val, 'pepg',
                            'PePG', colors['pepg'], styles['pepg'])
        _plot_algorithm_data(ax1, ax2, mdrr_by_param, param_val, 'mdrr',
                            'MDRR', colors['mdrr'], styles['mdrr'])
        _plot_algorithm_data(ax1, ax2, reg_pepg_by_param, param_val, 'reg_pepg',
                            'Reg PePG', colors['reg_pepg'], styles['reg_pepg'])

    style_axes(ax1, 'Iteration t', 'V^π', 'Value Function Evolution',
              fontsize=20, title_fontsize=22, tick_fontsize=16, legend_fontsize=14)
    style_axes(ax2, 'Iteration t', '||d_{t+1} - d_t||_2', 'Policy Stability',
              fontsize=20, title_fontsize=22, tick_fontsize=16, legend_fontsize=14)
...
def _plot_algorithm_data(
    ax1, ax2, data_by_param, param_val,
    algo_key, algo_label, color, style
):
    ...
    if 'd_diff_mean' in data and data['d_diff_mean']:
        ...
        plot_with_confidence_interval(
            ...
            color=color, label=algo_label,
            linestyle=linestyle, marker=marker, markersize=3,
            linewidth=2, alpha=0.2, semilogy=True
        )
...
def plot_individual_algorithm(
    ...
    for i, param_val in enumerate(param_values):
        ...
        if 'v_values_mean' in data_item and data_item['v_values_mean']:
            ...
            plot_with_confidence_interval(
                ...
                color=color, label=f'{param}={param_val}',
                marker='o', markersize=3, linewidth=2, alpha=0.3
            )
    ...
    style_axes(ax1, 'Iteration t', 'V^π',
              f'{algorithm_name} - Value Function Evolution',
              fontsize=20, title_fontsize=22, tick_fontsize=16, legend_fontsize=12)
    style_axes(ax2, 'Iteration t', '||d_{t+1} - d_t||_2',
              f'{algorithm_name} - Policy Stability',
              fontsize=20, title_fontsize=22, tick_fontsize=16, legend_fontsize=12)

    output_path = Path(output_dir) / f"individual_{algorithm_name.lower().replace(' ', '_')}_results"
    save_figure(output_path, dpi=300)
