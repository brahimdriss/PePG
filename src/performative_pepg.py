...
class PePG():
    def __init__(self, env: Gridworld, max_iterations, lamda, reg, gradient, eta, sampling, 
                 ...
                 feature_dim=32, nn_lr=0.001, warmup=1, num_trajectories=30, value_lr=0.1, seed=1):
    ...
    def retrain1(self):
        ...
        if self.iteration < self.warmup:
            ...
            gradient = self._compute_performative_gradient_with_learned_baseline(trajectories)
    ...
    def _compute_standard_gradient_with_learned_baseline(self, trajectories):
        ...
        for trajectory, returns, advantages in trajectory_data:
            for t, ((state, action, reward), advantage) in enumerate(zip(trajectory, advantages)):
        ...
        if self.iteration % 10 == 0 and all_returns:
            ...
            print(f"  REINFORCE w/ learned baseline: avg_return={avg_return:.4f}, avg_advantage={avg_advantage:.4f}, advantage_std={advantage_std:.4f}")
    ...
    def _compute_performative_gradient_with_learned_baseline(self, trajectories):
        ...
        for trajectory in trajectories:
            ...
            for t, ((state, action, reward), G_t) in enumerate(zip(trajectory, trajectory_returns)):
                discount = self.env.gamma ** t
                ...
                if t > 0:
                    ...
                    trans_grad = self._compute_transition_gradient(prev_state, prev_action, state)
